...
function getAverage(value1, value2) {
  // throw new Error('Not implemented');
  return (value1 + value2) / 2 === Infinity ? +1.7976931348623157e+308 : (value1 + value2) / 2;
}
...
function getLastDigit(value) {
  // throw new Error('Not implemented');
  return JSON.parse(JSON.stringify(value)[JSON.stringify(value).length - 1]);
}
...
function isPrime(num) {
  ...
  for (let i = 2; i <= Math.sqrt(num); i += 1) if (num % i === 0 && num !== i) return false;
  return true;
}
